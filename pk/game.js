...
const DIVE_DUR = 0.55, DIVE_MAX = 2.9;   // 飛びこみ
...
// kick: {tx,ty,pow} / keepers: [{x, dive:{dx,dy,len,t}|null}] (tはボール発射=0とした秒)
function simulate(kick, keepers) {
  const T = flightT(kick.pow);
  const H = arcH(kick.pow);
  const dt = 1 / 120;
  ...
  const ballP = (t) => {
    ...
    return new THREE.Vector3(kick.tx * s, BALL_R + (kick.ty - BALL_R) * s + H * 4 * s * (1 - s), SPOT_Z * (1 - s));
  };
  const keeperP = (k, t) => {
  ...
  for (let t = t0; t <= T + DIVE_DUR; t += dt) {
    ...
    for (let i = 0; i < ks.length; i++) for (let j = i + 1; j < ks.length; j++) {
      const a = ks[i], b = ks[j];
      if (a.pos.distanceTo(b.pos) < COLLIDE_D) {
...
const host = {
  ...
  cpuKick() {
    ...
    net.send({ t: "kick", mid: -1, tx, ty, pow: 0.55 + Math.random() * 0.4 });
  },
...
function handleMsg(msg) {
  switch (msg.t) {
    ...
    case "pos": {
      ...
      if (k && G.phase === "set") k.x = clamp(msg.x, -KEEPER_X_MAX, KEEPER_X_MAX);
      break;
    }
    case "kick": {
      if (G.kick) break;
      G.kick = { tx: msg.tx, ty: msg.ty, pow: msg.pow, launchAt: performance.now() + RUNUP * 1000 };
    ...
    case "dive": {
      ...
      k.dive = { dx: msg.dx, dy: msg.dy, len: msg.len, t };
      G.simCache = null;
      break;
    }
...
function startRoundView(msg) {
  ...
  setHint(meK ? "← 左右にドラッグでスタート位置をきめて! →" : "キーパーが位置についてるよ…");
  // 配置カウントダウン
...
function startAimView() {
  ...
  setHint(iAmKicker
    ? "スワイプの向き=コース、長さ=つよさ!つよすぎると枠外に…"
    : meK ? "位置ロック!タイミングを見てスワイプで飛びこめ(1回だけ)" : "どうなる!?");
}
...
// ────────────────── 入力(スワイプ/ドラッグ) ──────────────────
const ptr = { down: false, x0: 0, y0: 0, t0: 0, keeperX0: 0, moved: false };
let lastPosSend = 0;
...
layer.addEventListener("pointerup", (e) => {
  ...
  if (iAmKicker && G.phase === "aim" && !G.myKicked) {
    // キッカー: スワイプ→シュート
    G.myKicked = true;
    const tx = (dx / minDim) * 8;
    const ty = clamp((-dy / minDim) * 4.2, 0.05, 4.5);
    const pow = clamp(len / (0.6 * minDim), 0.3, 1);
    net.send({ t: "kick", tx, ty, pow });
  } else if (meK && (G.phase === "aim" || G.phase === "fly") && !G.myDove) {
    // キーパー: スワイプ→飛びこみ(画面右=世界の-x)
    G.myDove = true;
    const wx = -dx / len, wy = clamp(-dy / len, -0.25, 1);
    const L = Math.hypot(wx, wy) || 1;
    net.send({ t: "dive", dx: wx / L, dy: wy / L, len: clamp((len / (0.32 * minDim)) * DIVE_MAX, 0.7, DIVE_MAX) });
    setHint("");
  }
});
  ...
  if (G.phase !== "lobby") {
    ...
    G.keepers.forEach((k, i) => {
      ...
      if (k.dive) {
        ...
        y = Math.max(0, k.dive.dy * k.dive.len * e * 0.6 - p * p * 0.5);
    ...
    if (G.kick && tFly >= 0) {
      ...
      const t = Math.min(tFly, stopT);
      const s = clamp(t / T, 0, 1);
      const H = arcH(G.kick.pow);
      let bx = G.kick.tx * s;
      let by = BALL_R + (G.kick.ty - BALL_R) * s + H * 4 * s * (1 - s);
      let bz = SPOT_Z * (1 - s);
      if (tFly > stopT) {
...
window.__npk = { G, handleMsg, host, net, simulate };
